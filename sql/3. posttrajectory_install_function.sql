...
BEGIN
	...
	sql := 'SELECT id FROM static_partition WHERE ST_INTERSECTS(partition, $1);';
	RAISE DEBUG '%', sql;
	...
	IF (cnt_mpid < 1) THEN
		...
		EXECUTE 'INSERT INTO ' || quote_ident(f_trajectory_segtable_name) || '(mpid, segid, mpcount, partid, rect, start_time, end_time, tpseg) 
			VALUES($1, 1, 1, $2, box2d($3), TIMESTAMP '|| quote_literal(tp.ts) || ', TIMESTAMP  '|| quote_literal(tp.ts) || ', ARRAY[($3, TIMESTAMP ' || quote_literal(tp.ts) || ')]::tpoint[])'
	...
	IF(cnt_mpid > 0) THEN
		...
		IF(mp_seq.mpcount < tp_seg_size) THEN
			...
			IF(cnt < 1) THEN
				...
				EXECUTE 'INSERT INTO ' || quote_ident(f_trajectory_segtable_name) || '(mpid, segid, before_segid, mpcount, partid, rect, start_time, end_time, tpseg) 
					VALUES($1, ' || new_segid || ', $2, 2, $4, box2d($3), TIMESTAMP ' || quote_literal(tp.ts) || ' , TIMESTAMP ' || quote_literal(tp.ts) ||
		...
		ELSE 	
			...
			ELSE
				EXECUTE 'INSERT INTO ' || quote_ident(f_trajectory_segtable_name) ||'(mpid, segid, before_segid, mpcount, partid, rect, start_time, end_time, tpseg) 
					VALUES($1,  ' || new_segid+1 || ', $2, 1, $4, box2d($3), TIMESTAMP ' || quote_literal(tp.ts) || ' , TIMESTAMP ' || quote_literal(tp.ts) ||
