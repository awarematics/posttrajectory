...
--  FUNCTION ptraj_passes_base(trajectory, geometry)

DROP FUNCTION ptraj_passes_base(trajectory, geometry);

CREATE OR REPLACE FUNCTION ptraj_passes_base(trajectory, geometry) RETURNS boolean AS
$$
DECLARE
...
BEGIN		
	...
	sql_text := 'SELECT COUNT(*) FROM ' || traj_segtable_name || ' mp WHERE mp.mpid = ' || input_trajectory.moid || ' AND mp.rect TJ_Passes((mp).tpseg, $1)';
