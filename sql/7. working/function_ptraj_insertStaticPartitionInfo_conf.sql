...
/*
*
* select ptraj_insertStaticPartitionInfo_conf(10, 10);
*
*/


CREATE OR REPLACE FUNCTION ptraj_insertStaticPartitionInfo_conf(INTEGER, INTEGER) RETURNS BOOLEAN AS
$$
DECLARE
	x_split		alias for $1;
	y_split		alias for $2;
...
BEGIN
	...
	schemeName := 'public';
	tableName := 'static_partition_conf';
	...
	*/
	...
	FOR i IN 1..x_split LOOP
		FOR j IN 1..y_split LOOP
			...
			tmp_cell := 'ptraj.cell_' || (idx+j) || E' = \'Point(' || split_minX || ' ' || split_minY || '),Point(' || split_maxX || ' ' || split_maxY || E')\'';
			
			EXECUTE 'INSERT INTO ' || tableName || ' VALUES($1, $2)' USING (idx+j), tmp_cell;
		END LOOP;
		idx := i * y_split;
	END LOOP;
