...
-- FUNCTION DEFINITION
-- DESCRIPTION : 
-- NAME : TJ_slice_from_seg_v2(trajectory, geometry, periods)
-- PARAMETER : trajectory, geometry, periods
-- RETURNS : tpoint[]
-- CREATED BY YOO KI HYUN

-- START TJ_slice_from_seg_v2:

DROP FUNCTION IF EXISTS TJ_slice_from_seg_v2(trajectory, geometry, periods);

CREATE OR REPLACE FUNCTION TJ_slice_from_seg_v2(trajectory, geometry, periods) RETURNS tpoint[] AS
$$
DECLARE
...
*/
		...
		sql_text := 'SELECT mp.tpseg FROM ' || tmp_table || ' mp';				
		sql_text := sql_text || ' WHERE mp.mpid = ' || input_trajectory.moid;
